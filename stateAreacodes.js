var states = [
	    {
	    	longhand: "Alabama",
	    	abbrev: "AL",
	    	areacodes : [205, 251, 256, 334, 938]
	    },
	    {
	    	longhand: "Alaska",
	    	abbrev: "AK",
	    	areacodes : [907]
	    },
	    {
	    	longhand: "American Samoa",
	    	abbrev: "AS",
	    	areacodes : [684]
	    },
	    {
	    	longhand: "Arizona",
	    	abbrev: "AZ",
	    	areacodes : [480, 520, 602, 623, 928]
	    },
	    {
	    	longhand: "Arkansas",
	    	abbrev: "AR",
	    	areacodes : [479, 501, 870]
	    },
	    {
	    	longhand: "California",
	    	abbrev: "CA",
	    	areacodes : [209, 213, 310, 323, 408, 415, 424, 442, 510, 530, 559, 562, 619, 626, 628, 650, 657, 661, 669, 707, 714, 747, 760, 805, 818, 831, 858, 909, 916, 925, 949, 951]
	    },
	    {
	    	longhand: "Colorado",
	    	abbrev: "CO",
	    	areacodes : [303, 719, 720, 970]
	    },
	    {
	    	longhand: "Connecticut",
	    	abbrev: "CT",
	    	areacodes : [203, 475, 860, 959]
	    },
	    {
	    	longhand: "Delaware",
	    	abbrev: "DE",
	    	areacodes : [302]
	    },
	    {
	    	longhand: "Florida",
	    	abbrev: "FL",
	    	areacodes : [239, 305, 321, 352, 386, 407, 561, 727, 754, 772, 786, 813, 850, 863, 904, 941, 954]
	    },
	    {
	    	longhand: "Georgia",
	    	abbrev: "GA",
	    	areacodes : [229, 404, 470, 478, 678, 706, 762, 770, 912]
	    },
	    {
	    	longhand: "Guam",
	    	abbrev: "GU",
	    	areacodes : [671]
	    },
	    {
	    	longhand: "Hawaii",
	    	abbrev: "HI",
	    	areacodes : [808]
	    },
	    {
	    	longhand: "Idaho",
	    	abbrev: "ID",
	    	areacodes : [208]
	    },
	    {
	    	longhand: "Illinois",
	    	abbrev: "IL",
	    	areacodes : [217, 224, 309, 312, 331, 618, 630, 708, 773, 779, 815, 847, 872]
	    },
	    {
	    	longhand: "Indiana",
	    	abbrev: "IN",
	    	areacodes : [219, 260, 317, 463, 574, 765, 812, 930]
	    },
	    {
	    	longhand: "Iowa",
	    	abbrev: "IA",
	    	areacodes : [319, 515, 563, 641, 712]
	    },    
	    {
	    	longhand: "Kansas",
	    	abbrev: "KS",
	    	areacodes : [316, 620, 785, 913]
	    },    
	    {
	    	longhand: "Kentucky",
	    	abbrev: "KY",
	    	areacodes : [270, 364, 502, 606, 859]
	    },
	    {
	    	longhand: "Louisiana",
	    	abbrev: "LA",
	    	areacodes : [225, 318, 337, 504, 985]
	    },
	    {
	    	longhand: "Maine",
	    	abbrev: "ME",
	    	areacodes : [207]
	    },
	    {
	    	longhand: "Maryland",
	    	abbrev: "MD",
	    	areacodes : [240, 301, 410, 443, 667]
	    },  
	    {
	    	longhand: "Massachusetts",
	    	abbrev: "MA",
	    	areacodes : [339, 351, 413, 508, 617, 774, 781, 857, 978]
	    },  
	    {
	    	longhand: "Michigan",
	    	abbrev: "MI",
	    	areacodes : [231, 248, 269, 313, 517, 586, 616, 734, 810, 906, 947, 989]
	    },  
	    {
	    	longhand: "Minnesota",
	    	abbrev: "MN",
	    	areacodes : [218, 320, 507, 612, 651, 763, 952]
	    },  
	    {
	    	longhand: "Mississippi",
	    	abbrev: "MS",
	    	areacodes : [228, 601, 662, 769]
	    },  
	    {
	    	longhand: "Missouri",
	    	abbrev: "MO",
	    	areacodes : [314, 417, 573, 636, 660, 816]
	    },
	    {
	    	longhand: "Montana",
	    	abbrev: "MT",
	    	areacodes : [406]
	    },
	    {
	    	longhand: "Nebraska",
	    	abbrev: "NE",
	    	areacodes : [308, 402, 531]
	    },
	    {
	    	longhand: "Nevada",
	    	abbrev: "NV",
	    	areacodes : [702, 725, 775]
	    },
	    {
	    	longhand: "New Hampshire",
	    	abbrev: "NH",
	    	areacodes : [603]
	    },
	    {
	    	longhand: "New Jersey",
	    	abbrev: "NJ",
	    	areacodes : [201, 551, 609, 732, 848, 856, 862, 908, 973]
	    },
	    {
	    	longhand: "New Mexico",
	    	abbrev: "NM",
	    	areacodes : [505, 575]
	    },
	    {
	    	longhand: "New York",
	    	abbrev: "NY",
	    	areacodes : [212, 315, 332, 347, 516, 518, 585, 607, 631, 646, 680, 716, 718, 845, 914, 917, 929, 934]
	    },
	    {
	    	longhand: "North Carolina",
	    	abbrev: "NC",
	    	areacodes : [252, 336, 704, 743, 828, 910, 919, 980, 984]
	    },
	    {
	    	longhand: "North Dakota",
	    	abbrev: "ND",
	    	areacodes : [701]
	    },
	    {
	    	longhand: "Northern Mariana Islands",
	    	abbrev: "MP",
	    	areacodes : [670]
	    },
	    {
	    	longhand: "Ohio",
	    	abbrev: "OH",
	    	areacodes : [216, 220, 234, 330, 380, 419, 440, 513, 567, 614, 740, 937]
	    },
	    {
	    	longhand: "Oklahoma",
	    	abbrev: "OK",
	    	areacodes : [405, 539, 580, 918]
	    },
	    {
	    	longhand: "Oregon",
	    	abbrev: "OR",
	    	areacodes : [458, 503, 541, 971]
	    },
	    {
	    	longhand: "Pennsylvania",
	    	abbrev: "PA",
	    	areacodes : [215, 267, 272, 412, 484, 570, 610, 717, 724, 814, 878]
	    },
	    {
	    	longhand: "Puerto Rico",
	    	abbrev: "PR",
	    	areacodes : [787, 939]
	    },
	    {
	    	longhand: "Rhode Island",
	    	abbrev: "RI",
	    	areacodes : [401]
	    },
	    {
	    	longhand: "South Carolina",
	    	abbrev: "SC",
	    	areacodes : [803, 843, 854, 864]
	    },
	    {
	    	longhand: "South Dakota",
	    	abbrev: "SD",
	    	areacodes : [605]
	    },
	    {
	    	longhand: "Tennessee",
	    	abbrev: "TN",
	    	areacodes : [423, 615, 629, 731, 865, 901, 931]
	    },
	    {
	    	longhand: "Texas",
	    	abbrev: "TX",
	    	areacodes : [210, 214, 254, 281, 325, 346, 361, 409, 430, 432, 469, 512, 682, 713, 737, 806, 817, 830, 832, 903, 915, 936, 940, 956, 972, 979]
	    },
	    {
	    	longhand: "Utah",
	    	abbrev: "UT",
	    	areacodes : [385, 435, 801]
	    },
	    {
	    	longhand: "Vermont",
	    	abbrev: "VT",
	    	areacodes : [802]
	    },
	    {
	    	longhand: "Virgin Islands",
	    	abbrev: "VI",
	    	areacodes : [340]
	    },
	    {
	    	longhand: "Virginia",
	    	abbrev: "VA",
	    	areacodes : [276, 434, 540, 571, 703, 757, 804]
	    },
	    {
	    	longhand: "Washington",
	    	abbrev: "WA",
	    	areacodes : [206, 253, 360, 425, 509]
	    },
	    {
	    	longhand: "Washington, DC",
	    	abbrev: "DC",
	    	areacodes : [202]
	    },
	    {
	    	longhand: "West Virginia	",
	    	abbrev: "WV",
	    	areacodes : [304, 681]
	    },
	    {
	    	longhand: "Wisconsin",
	    	abbrev: "WI",
	    	areacodes : [262, 414, 534, 608, 715, 920]
	    },
	    {
	    	longhand: "Wyoming",
	    	abbrev: "WY",
	    	areacodes : [307]
	    }
    ];